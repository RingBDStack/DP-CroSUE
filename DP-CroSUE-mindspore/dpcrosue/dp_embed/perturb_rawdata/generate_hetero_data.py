...
def arg_parser():
    ...
    parser.add_argument('--n_eigenvector',
                        type=int,
                        default=128,
                        help='use eigenvector as initial feature.')
...
def set_data_args(dataset, args):
    if dataset == "foursquare":
        ...
        args.attr_epsilon = 5
        args.save_perturb_attr_path = args.perturb_data_folder + "userattr-{}".format(
            str(args.attr_epsilon))
        # user texts
        ...
        # args.multi_label_path = args.exp_folder + "jointmultilabel.pkl"
        ...
        # user attribute
        args.user_attr_file = args.exp_folder + "user_info" + dataset[
            -1] + ".csv"
        args.save_ori_attr_path = args.ori_data_folder + "userattr"
        args.attr_epsilon = 5
        args.save_perturb_attr_path = args.perturb_data_folder + "userattr-{}".format(
        ...
        args.text_data_file = "../../dataset/weibo/cleaned_blog%s.csv" % dataset[
            -1]
        ...
        # userfollow
        args.follow_eplison = 15
        args.user_follow_file = "../../dataset/weibo/newrel" + dataset[
            -1] + ".npy"
        args.label_path = args.exp_folder + "/user_label%s.pkl" % dataset[-1]
        args.multi_label_path = args.exp_folder + "/multilabel.pkl"
        args.label_dir = args.exp_folder + "data/" + dataset + "/"
        args.occu_label_path = args.exp_folder + "user_occu_label{}.pkl".format(
            dataset[-1])
        args.gender_label_path = args.exp_folder + "user_gender_label{}.pkl".format(
            dataset[-1])
...
def make_data_embedding(dataset):
    ...
    if not os.path.exists(args.save_perturb_attr_path + '.npy'):
        ...
        make_user_attr(args.dataset, args.user_attr_file, args.occu_label_path,
                       args.gender_label_path, args.attr_epsilon,
                       args.save_ori_attr_path, args.save_perturb_attr_path)
    else:
        print("orginal user attribute file:", args.save_ori_attr_path + '.npy')
        print("perturbed user attribute file:",
              args.save_perturb_attr_path + '.npy', '\n')
    ...
    if not os.path.exists(args.out_textvec_file + '.pkl'):
        ...
        make_user_text_vec(args.dataset, args.text_file, args.usertextfile,
                           args.out_textvec_file)
        convert_label_textvec(user2id, args.out_textvec_file + '.pkl',
                              args.ori_data_folder)

    else:
        print('original user text vector file:',
              args.out_textvec_file + '.pkl')
        convert_label_textvec(user2id, args.out_textvec_file + '.pkl',
                              args.ori_data_folder)

    if not os.path.exists(args.ori_data_folder + 'following.npy'):
        convert_userfollowing(args.dataset, user2id, args.user_follow_file,
                              args.ori_data_folder)
    ...
    convert_label_textvec(user2id, args.multi_label_path, args.label_dir)
