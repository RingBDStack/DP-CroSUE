...
def construct_heter_graph_from_file(userfollowfile, usertextvecfile,
                                    userattrfile):
    ...
    G = dgl.heterograph({
        ('user', 'hasfan', 'user'): (torch.tensor(dst), torch.tensor(src)),
    ...
    G.nodes['texts'].data['fea'] = torch.tensor(np.array(usertextfea))
...
def construct_joint_heter_graph_from_file(userfollowfileA, usertextvecfileA,
                                          userattrfileA, userfollowfileB,
                                          usertextvecfileB, userattrfileB,
                                          indexA, indexB):
    srcA, dstA, usertextidA, usertextfeaA, userfeaA = get_graph_eles(
        userfollowfileA, usertextvecfileA, userattrfileA)
    srcB, dstB, usertextidB, usertextfeaB, userfeaB = get_graph_eles(
        userfollowfileB, usertextvecfileB, userattrfileB)

    usernumA = np.max(srcA) + 1 if np.max(srcA) > np.max(
        dstA) else np.max(dstA) + 1
    ...
    dst = np.concatenate(
        [indexB + np.array([usernumA] * indexB.shape[0]), dst], axis=0)
...
def splitdata(labeldict, train_per, val_per):
    ...
    label = torch.tensor(np.array(list(label_dict.values())),
                         dtype=torch.int64)
...
def get_triplet(G, nodesnum, size):
    edges = torch.cat([
        G.edges(etype='hasfan')[0].unsqueeze(dim=1),
        G.edges(etype='hasfan')[1].unsqueeze(dim=1)
    ],
                      dim=1)
