...
from Network_Release.src.utils import graph_to_adj_list

dir_path = os.path.dirname(os.path.realpath(__file__))
root_path = os.path.abspath(os.path.join(dir_path, os.pardir))

multi_graph_dataset = set(['weibo1','weibo2','twitter','foursquare','relabeled_dblp2', 'new_dblp2', 'dblp2','new_IMDB_MULTI', 'IMDB_MULTI', 'Resampled_IMDB_MULTI'])
...
class Single_Graph_Dataset:
    ...
    def load_data(self):
        if self.dataset_str in multi_graph_dataset:
            adj = self.graph_adj
        else:
