...
multi_graph_dataset = set(['foursquare','twitter','relabeled_dblp2', 'new_dblp2', 'dblp2','new_IMDB_MULTI', 'IMDB_MULTI', 'Resampled_IMDB_MULTI'])
...
if __name__ == '__main__':
    ...
    for counter, dataset in enumerate(datasets.datasets):
        ...
        print(generated_adj)
        print("graph G type:", type(G))
        print("graph G:",G)
        generated_graph_list.append(G)


    # if args.save_generated_graph:
    #    print("save_generated_graph is ture!")
    save_data(args, generated_graph_list[0])
    logger = stat_logger(args, current_time)
