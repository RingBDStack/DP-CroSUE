...
def build_model(params, with_dis):
    ...
    # with open(params.tgt_emb, 'r') as fp:
    #     receive_data = json.load(fp)
    #     _tgt_emb = torch.from_numpy(np.array(receive_data['ent_embeddings']))
    # with open(params.src_emb, 'r') as fp:
    #     send_data = json.load(fp)
    #     _src_emb = torch.from_numpy(np.array(send_data['ent_embeddings']))

    _tgt_emb = torch.load(params.tgt_emb)
    _src_emb = torch.load(params.src_emb)

    params.emb_dim = _src_emb.shape[1]
    src_emb = nn.Embedding(_src_emb.shape[0], params.emb_dim, sparse=True)
    src_emb.weight.data.copy_(_src_emb)
    tgt_emb = nn.Embedding(_tgt_emb.shape[0],params.emb_dim,sparse=True)
    tgt_emb.weight.data.copy_(_tgt_emb)
    # mapping
    ...
    params.src_mean = normalize_embeddings(src_emb, params.normalize_embeddings)
    if params.tgt_lang:
        params.tgt_mean = normalize_embeddings(tgt_emb, params.normalize_embeddings)
